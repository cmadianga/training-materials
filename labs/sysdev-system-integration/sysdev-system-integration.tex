...
In order to avoid some issues when we will try to boot our board with
systemd activated, we need to disable \kconfig{CONFIG_DRM_ETNAVID} feature from
linux kernel thanks to a fragment file.

A fragment file is a list of kernel configuration that will be merged to the main kernel configuration file.

To address this, le'ts copy this fragment file to \code{board/bootlin/training/}:

\begin{bashinput}
cp ../data/fragment-01-linux-cleanup.config board/bootlin/training
\end{bashinput}
\else
...
\item \code{Toolchain}
  ...
  \item \code{Bootlin toolchain variant}: \ifdefstring{\labboardarch}{64bit}
...
\item \code{Kernel}
  ...
  \item Set \code{URL of Custom repository} to \code{https://github.com/STMicroelectronics/linux}
  \item Set \code{Custom repository version} to \code{v6.6-stm32mp}
  \else
  \item Set \code{Kernel version} to \code{Custom version}
  ...
  \ifdefstring{\labboard}{stm32mp2}{\item Set \code{Additional configuration fragment files} to \code{board/bootlin/training/fragment-01-linux-cleanup.config}}{}
  ...
  \ifdefstring{\labboardarch}{64bit}{\item Set \code{Kernel Binary format} to \code{Image.gz}}{}
  \end{itemize}
