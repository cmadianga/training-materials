...
\begin{center}
\includegraphics[width=\textwidth]{labs/kernel-compiling-and-nfs-booting-stm32mp2/host-vs-target.pdf}
\end{center}
...
\end{itemize}
\item Enable the below items to add Ethernet support in our kernel version:
\begin{itemize}
\item \kconfig{CONFIG_STMMAC_ETH}
\item \kconfig{CONFIG_STMMAC_PLATFORM}
\item \kconfig{CONFIG_DWMAC_GENERIC}
\item \kconfig{CONFIG_DWMAC_STM32}
\end{itemize}
\end{itemize}
...
Now, copy the \code{Image.gz} and \code{stm32mp257f-dk.dtb} files to the
...
\scriptsize
\begin{verbatim}
/home/<user>/__SESSION_NAME__-labs/modules/nfsroot 192.168.0.100(rw,no_root_squash,no_subtree_check)
\end{verbatim}
\normalsize
...
\begin{verbatim}
setenv bootargs root=/dev/nfs rw ip=192.168.0.100:::::eth0 console=ttySTM0,115200n8
  nfsroot=192.168.0.1:/home/<user>/linux-kernel-stm32mp2-labs/modules/nfsroot,nfsvers=3,tcp
\end{verbatim}
...
{\footnotesize
\begin{verbatim}
setenv bootcmd 'tftp 0x8a000000 Image.gz; tftp 0x90000000 stm32mp257f-dk.dtb; booti 0x8a000000 - 0x90000000'
saveenv
\end{verbatim}
}
...
\begin{verbatim}
make savedefconfig
cp defconfig arch/arm64/configs/stm32mp2_defconfig
\end{verbatim}
