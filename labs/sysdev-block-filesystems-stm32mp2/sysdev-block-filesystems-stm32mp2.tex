...
Now, check the contents of \code{/proc/filesystems}. You should see
 ...
 \section{Support only one mmc bus instance}

To avoid any misunderstanding during this lab,
as SDMMC instance probing can vary between boot,
let's disable the SDMMC instances not related to the microSD card.

If you check the STM32MP257F Discovery kit User manual
\footnote{\url{https://www.st.com/resource/en/user_manual/um3385-discovery-kit-with-stm32mp257f-mpu-stmicroelectronics.pdf}},
you will see that SDMMC1 handles the microSD Card when SDMMC2 and SDMMC3
are respectively used for eMMC and Wi-Fi.
So, to disable SDMMC2 and SDMMC3 in the custom Device Tree for our board,
we have to add the below lines:
...
\item One partition, for the root filesystem, 256 MB big:
\begin{verbatim}
mkpart rootfs 135168s 659455s
\end{verbatim}

\item One partition, that fills the rest of the SD card, that will be
  used for the data filesystem:
\begin{verbatim}
mkpart data 659456s 100%
\end{verbatim}
\end{itemize}
...
\begin{ubootinput}
=> load mmc 0:4 %\zimageboardaddr% Image.gz
