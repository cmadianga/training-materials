...
\ifdefstring{\labboardarch}{64bit}{\code{aarch64-unknown-linux-uclibc}}{}%
...
In \code{Target options}:
\begin{itemize}
\item Set \code{Emit assembly for CPU} (\code{ARCH_CPU}) to \code{cortex-a35}.
...
\ifdefstring{\labboardarch}{64bit}{\code{$HOME/x-tools/aarch64-training-linux-musl/bin/}}
...
\ifdefstring{\labboardarch}{64bit}{\code{aarch64-linux-gcc}}{\code{arm-linux-gcc}}:

\begin{bashinput}
$ %\ifdefstring{\labboardarch}{64bit}{aarch64-linux-gcc}{arm-linux-gcc}% -o hello hello.c
\end{bashinput}
You can use the \code{file} command on your binary to make sure it has
correctly been compiled for the \ifdefstring{\labboardarch}{64bit}{AARCH64}{ARM}
architecture.
...
Now, try to run QEMU \ifdefstring{\labboardarch}{64bit}{AARCH64}{ARM}
user emulator:

\if\defstring{\labboardarch}{64bit}
\begin{bashinput}
$ qemu-aarch64 hello
...
What's happening is that \ifdefstring{\labboardarch}{64bit}{\code{qemu-aarch64}}
{\code{qemu-arm}} is missing the shared library loader
(compiled for \ifdefstring{\labboardarch}{64bit}{AARCH64}{ARM}) that this
binary relies on. Let's find it in our newly compiled toolchain:

\begin{bashinput}
$ find ~/x-tools -name %\ifdefstring{\labboardarch}{64bit}{ld-musl-aarch64.so.1}{ld-musl-armhf.so.1}%
\end{bashinput}
...
We can now use the \code{-L} option of \ifdefstring{\labboardarch}{64bit}
