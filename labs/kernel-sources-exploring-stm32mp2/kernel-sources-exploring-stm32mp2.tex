...
\item Create a branch based on a remote tree to explore a particular
      stmicro kernel version (from the \code{stmicro} kernel tree).
...
\section{Choose a particular stmicro version}

Let's work with a particular stmicro version of the Linux kernel.
...
First, let's get the list of branches on our remote trees:
...
As we will do our labs with the Linux 6.6 stable branch, the remote branch
we are interested in is \code{remotes/stmicro/v6.6-stm32mp}.
...
\begin{verbatim}
git checkout -b bootlin-labs stmicro/v6.6-stm32mp
\end{verbatim}

Check the version again using the \code{make kernelversion} command
to make sure you now have a 6.6.y version.
