...
Before continuing, use the configure file provided in the data directory to
reconfigure the linux kernel and rebuild it (remember that the Linux kernel
configuration file is .config in the linux kernel sources)
...
\begin{itemize}
  \item Connect one end to pin Z8 of GPIO connector CN5
  \item Connect the other end to pin 39 (GND) of GPIO connector CN5
\end{itemize}

\includegraphics[width=6cm]{labs/sysdev-accessing-hardware-stm32mp2/stm32mp257f-dk-board-gpio-connected-to-gnd.jpg}

If you check the {\em Pinout of the GPIO connector} in the board
documentation
\footnote{\url{https://www.st.com/resource/en/user_manual/um3385-discovery-kit-with-stm32mp257f-mpu-stmicroelectronics.pdf}},
you will see that the PZ8 is actually a GPIO pin on GPIO bank Z, and is configured
...
find that GPIO bank Z corresponds to \code{gpiochip9} and to GPIO
numbers 648 to 657. Hence, PZ8, the ninth pin on this bank corresponds to
GPIO number \gpionum.
...
\code{PZ8} file in \code{/sys/class/gpio}.
...
{Now, let's connect our GPIO pin to pin 1 (IOREF 3V3) of connector CN5.
That's the same connector as before, just the first pin from the top
(in the board picture above), instead of the thirty-ninth one.
...
same document about the board, you will find that I2C2 and I2C8 are
conveniently available through the GPIO connector. Let's try
to use one of them as for example I2C8!
...
\code{0x40130000}, which is \code{I2C2} in the STM32MP2
nomenclature. Refer to the STM32MP2 memory map in the datasheet for
details. Pay attention to the numbering difference: \code{i2c-0},
is the Linux numbering, based on the registration order
of enabled I2C busses. Here, because only I2C2 is enabled,
it is called \code{i2c-0}.

Using the datasheet for the SoC
\footnote{\url{https://www.st.com/resource/en/reference_manual/rm0457-stm32mp2325xx-advanced-armbased-3264bit-mpus-stmicroelectronics.pdf}},
...
Fortunately, I2C8 is already defined in the Device Tree for our board.
Look by yourself in this file \kfile{arch/arm64/boot/dts/st/stm32mp257f-dk.dts},
and you will find its definition, but with \code{status = "disabled";}.
...
new \code{arch/arm/boot/dts/st/stm32mp157f-dk-custom.dts} file containing:
...
We can see four devices here at adresses 0x38, 0x3c, 0x3d and 0x3f,
indicated by \code{UU}, which means that there is a kernel driver actively
driving these devices.

Now try to probe I2C8 through \code{i2cdetect -r 1}.
...
\includegraphics[width=12cm]{labs/sysdev-accessing-hardware-stm32mp2/stm32mp2-dk-nunchuk-connected.png}
\begin{center}
\includegraphics[width=6cm]{labs/sysdev-accessing-hardware-stm32mp2/stm32mp2-dk-board-nunchuk-connected.jpg}
\end{center}

\begin{itemize}
\item Connect the Nunchuk PWR pin to pin 1 (3V3) of GPIO connector CN5
\item Connect the Nunchuk GND pin to pin 39 (GND) of GPIO connector CN5
\item Connect the Nunchuk SCL pin to pin 5 (PZ4 - I2C8\_SCL) of GPIO connector CN5
\item Connect the Nunchuk SDA pin to pin 3 (PZ3 - I2C8\_SDA) of GPIO connector CN5
\end{itemize}
...
Bus 001 Device 004: ID 0951:1666
\end{bashinput}
...
# lsusb
Bus 001 Device 001: ID 1d6b:0002
Bus 001 Device 007: ID 0a12:4010 
Bus 001 Device 002: ID 0424:2422 
Bus 001 Device 008: ID 0b0e:24bb
Bus 001 Device 004: ID 0951:1666 
\end{bashinput}

The device of vendor ID \code{0b0e} and product ID \code{24bb} has
...
\begin{bashinput}
usb 1-1.1.1: new full-speed USB device number 8 using ehci-platform
\end{bashinput}

So if we go in \code{/sys/bus/usb/devices/1-1.1.1}, we get the {\em
sysfs} representation of this USB device:

\begin{bashinput}
# cd /sys/bus/usb/devices/1-1.1.1
# cat idVendor
0b0e
# cat idProduct
24bb
...
# cat /proc/asound/cards
 0 [J85            ]: USB-Audio - Jabra Evolve2 85
                      Jabra Evolve2 85 at usb-482f0000.usb-1.1.1, full speed
\end{bashinput}
...
\begin{bashinput}
# modprobe nunchuk
[   17.039092] nunchuk: loading out-of-tree module taints kernel.
[   17.041682] input: Wii Nunchuk as /devices/platform/soc@0/42080000.bus/46040000.i2c/i2c-1/1-0052/input/input5
[   17.049438] Nunchuk device probed successfully
\end{bashinput}
...
(\code{event5} for \code{input5} for example):

\begin{bashinput}
# cat /dev/input/event5 | od -x
\end{bashinput}
...
\code{STM32MP2 media player}. Don't hesitate to ask your
...
\texttt{git format-patch stmicro/v6.6-stm32mp}
