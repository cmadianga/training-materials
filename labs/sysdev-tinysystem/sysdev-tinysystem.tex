...
Add options to support Ethernet device as a feature in the kernel image instead of as a kernel module:
\begin{itemize}
  \item \kconfigval{CONFIG_STMMAC_ETH}{y}
  \item \kconfigval{CONFIG_STMMAC_PLATORM}{y}
  \item \kconfigval{CONFIG_DWMAC_GENERIC}{y}
  \item \kconfigval{CONFIG_DWMAC_STM32}{y}
\end{itemize}
}{}

Also check that \kconfig{CONFIG_DEVTMPFS_MOUNT} is
enabled (we will explain it later in this lab).
\ifdefstring{\labboard}{stm32mp2}{Rebuild your kernel}{If necessary, rebuild
your kernel.}
...
\begin{verbatim}
VFS: Mounted root (nfs filesystem) on device 0:21.
\end{verbatim}
  \else
\begin{verbatim}
...
So, what's missing is the \ifdefstring{\labboardarch}{64bit}{\code{/lib/ld-musl-aarch64.so.1}}{\code{/lib/ld-musl-armhf.so.1}}
