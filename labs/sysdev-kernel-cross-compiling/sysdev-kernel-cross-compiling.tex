...
\begin{itemize}
\item Checkout a \ifdefstring{\labboard}{stm32mp2}{particular}{stable} version of the Linux kernel
...
\section{Choose a particular \ifdefstring{\labboard}{stm32mp2}{version}{stable version} of Linux}

We will use \texttt{linux-\workingkernel.x},
\ifdefstring{\labboard}{qemu}{}{%
  \ifdefstring{\labboardarch}{64bit}{}{which corresponds to an LTS release, and}
}
...
\texttt{remotes/\ifdefstring{\labboard}{stm32mp2}{stmicro}{stable}/\ifdefstring{\labboard}{stm32mp2}{v6.6-stm32mp}{linux-\workingkernel.y}}.
...
\begin{bashinput}
git checkout stmicro/v6.6-stm32mp
\end{bashinput}
\else
\begin{bashinput}
...
\begin{bashinput}
$ export PATH=$HOME/x-tools/%\ifdefstring{\labboardarch}{64bit}{aarch64-training-linux-musl}{arm-training-linux-musleabihf}%/bin:$PATH
\end{bashinput}
...
\end{itemize}
\ifdefstring{\labboardarch}{64bit}{
...
\item In the \code{Platform Selection} menu, remove support for all the SoCs except
for the STMicroelectronis STM32 SoC Family.
\item Disable \kconfig{CONFIG_DRM}, which will skip support for many display
controller and GPU drivers.
\item Disable the below items to avoid compilation issue with our kernel version:
\begin{itemize}
\item \kconfig{CONFIG_MLX5_CORE}
\item \kconfig{CONFIG_TOUCHSCREEN_EDT_FT5X06}
\item \kconfig{CONFIG_TOUCHSCREEN_GOODIX}
\end{itemize}
}{}
\end{itemize}
...
optimized kernel for the \ifdefstring{\labboard}{beagleplay}{BeaglePay}{STM32MP257F-DK}: the ARM64 \code{defconfig}
enables plenty of features and drivers that will not be useful on our
particular board.}{}
...
\item On your workstation, copy the {\tt \ifdefstring{\labboardarch}{64bit}{Image.gz}{zImage}} and DTB
(\texttt{\dtname}\texttt{.dtb}) to the directory exposed by the TFTP server.

\item On the target (in the U-Boot prompt), load {\tt \ifdefstring{\labboardarch}{64bit}{Image.gz}{zImage}} from
TFTP into RAM:
\begin{ubootinput}
=> tftp %\zimageboardaddr% %\ifdefstring{\labboardarch}{64bit}{Image.gz}{zImage}%
\end{ubootinput}
...
\begin{ubootinput}
=> %\ifdefstring{\labboardarch}{64bit}{booti}{bootz} \zimageboardaddr% - %\dtbboardaddr%
\end{ubootinput}
...
\begin{ubootinput}
=> setenv bootcmd 'tftp %\zimageboardaddr\ \ifdefstring{\labboardarch}{64bit}{Image.gz}{zImage}; tftp \dtbboardaddr\ {\dtname}.dtb; \ifdefstring{\labboardarch}{64bit}{booti}{bootz} \zimageboardaddr\ - \dtbboardaddr'%
=> saveenv
\end{ubootinput}
